...
def write_conda_recipe(version):
    """
    Writes the rayml recipe to build a conda package based on the version
    Args:
        version: The version of rayml we are building with this feedstock
    ...
    recipe_file_path = os.path.join(
        os.path.join(os.path.join(os.getcwd(), "rayml-core-feedstock"), "recipe"),
        "meta" ".yaml",
    )
    with open(recipe_file_path, "rb") as config_file:
        ...
        # Path to the rayml repository on the docker container.  Since we are doing a local build this is
        # the target we are copying this directory to.
        recipe_path = str(pathlib.Path("..", "feedstock_root", "rayml"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("version", help="The version of rayml being built")
