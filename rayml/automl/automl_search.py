...
from rayml.automl.automl_algorithm import DefaultAlgorithm, IterativeAlgorithm
from rayml.automl.callbacks import log_error_callback
from rayml.automl.engine import SequentialEngine
from rayml.automl.engine.cf_engine import CFClient, CFEngine
from rayml.automl.engine.dask_engine import DaskEngine
...
from rayml.data_checks import DataCheckMessageType, DefaultDataChecks
...
from rayml.tuners import SKOptTuner
from rayml.utils import convert_to_seconds, infer_feature_types
from rayml.utils.gen_utils import contains_all_ts_parameters
from rayml.utils.logger import (
    get_logger,
    log_subtitle,
    log_title,
    time_elapsed,
...
def search(
    ...
    This method will run rayml's default suite of data checks. If the data checks produce errors, the data check results will be returned before running the automl search. In that case we recommend you alter your data to address these errors and try again.
    ...
    Args:
        ...
        problem_type (str or ProblemTypes): Type of supervised learning problem. See rayml.problem_types.ProblemType.all_problem_types for a full list.
...
class AutoMLSearch:
    ...
    Args:
        ...
        allowed_model_families (list(str, ModelFamily)): The model families to search. The default of None searches over all
            model families. Run rayml.pipelines.components.utils.allowed_model_families("binary") to see options. Change `binary`
    ...
    def __init__(
        ...
        if is_time_series(self.problem_type):
            warnings.warn(
                "Time series support in rayml is still in beta, which means we are still actively building "
        ...
        if isinstance(objective, type):
            if objective in non_core_objectives:
                raise ValueError(
                    ...
                    "Use rayml.objectives.utils.get_core_objective_names() "
