...
from rayml.model_understanding._partial_dependence import (
    _partial_dependence,
    _range_for_dates,
)
from rayml.model_understanding.permutation_importance import (
    calculate_permutation_importance,
)
from rayml.objectives.utils import get_objective
from rayml.problem_types import ProblemTypes
from rayml.utils import import_or_raise, infer_feature_types, jupyter_check
...
def partial_dependence(
    ...
    try:
        ...
        classes = None
        if isinstance(pipeline, rayml.pipelines.BinaryClassificationPipeline):
            classes = [pipeline.classes_[1]]
        elif isinstance(pipeline, rayml.pipelines.MulticlassClassificationPipeline):
...
def graph_partial_dependence(
    ...
    if jupyter_check():
        import_or_raise("ipywidgets", warning=True)
    if (
        isinstance(pipeline, rayml.pipelines.MulticlassClassificationPipeline)
    ...
    if isinstance(pipeline, rayml.pipelines.MulticlassClassificationPipeline):
