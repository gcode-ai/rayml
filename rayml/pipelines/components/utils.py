"""Utility methods for rayml components."""
import inspect
...
from rayml.model_family.utils import ModelFamily, handle_model_family
from rayml.pipelines.components.component_base import ComponentBase
from rayml.pipelines.components.estimators.estimator import Estimator
from rayml.pipelines.components.transformers.transformer import Transformer
from rayml.problem_types import ProblemTypes, handle_problem_types
from rayml.utils import get_importable_subclasses
...
def handle_component_class(component_class):
    ...
    Examples:
        >>> from rayml.pipelines.components.estimators.regressors.decision_tree_regressor import DecisionTreeRegressor
        >>> handle_component_class(DecisionTreeRegressor)
        <class 'rayml.pipelines.components.estimators.regressors.decision_tree_regressor.DecisionTreeRegressor'>
        >>> handle_component_class("Random Forest Regressor")
        <class 'rayml.pipelines.components.estimators.regressors.rf_regressor.RandomForestRegressor'>
    """
...
class WrappedSKClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, pipeline):
        """Scikit-learn classifier wrapper class. Takes an rayml pipeline as input and returns a scikit-learn classifier class wrapping that pipeline.

        Args:
            pipeline (PipelineBase or subclass obj): rayml pipeline.
        """
...
class WrappedSKRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, pipeline):
        """Scikit-learn regressor wrapper class. Takes an rayml pipeline as input and returns a scikit-learn regressor class wrapping that pipeline.
...
def scikit_learn_wrapped_estimator(rayml_obj):
    """Wraps an rayml object as a scikit-learn estimator."""
    from rayml.pipelines.pipeline_base import PipelineBase

    """Wrap an rayml pipeline or estimator in a scikit-learn estimator."""
    if isinstance(rayml_obj, PipelineBase):
        if rayml_obj.problem_type in [
            ProblemTypes.REGRESSION,
            ProblemTypes.TIME_SERIES_REGRESSION,
        ]:
            return WrappedSKRegressor(rayml_obj)
        elif (
            rayml_obj.problem_type == ProblemTypes.BINARY
            or rayml_obj.problem_type == ProblemTypes.MULTICLASS
        ):
            return WrappedSKClassifier(rayml_obj)
    else:
        # rayml Estimator
        if rayml_obj.supported_problem_types == [
            ProblemTypes.REGRESSION,
            ProblemTypes.TIME_SERIES_REGRESSION,
        ]:
            return WrappedSKRegressor(rayml_obj)
        elif rayml_obj.supported_problem_types == [
            ProblemTypes.BINARY,
            ...
            return WrappedSKClassifier(rayml_obj)
    raise ValueError("Could not wrap rayml object in scikit-learn wrapper.")


def generate_component_code(element):
    r"""Creates and returns a string that contains the Python imports and code required for running the rayml component.
    ...
    Examples:
        >>> from rayml.pipelines.components.estimators.regressors.decision_tree_regressor import DecisionTreeRegressor
        >>> assert generate_component_code(DecisionTreeRegressor()) == "from rayml.pipelines.components.estimators.regressors.decision_tree_regressor import DecisionTreeRegressor\n\ndecisionTreeRegressor = DecisionTreeRegressor(**{'criterion': 'mse', 'max_features': 'auto', 'max_depth': 6, 'min_samples_split': 2, 'min_weight_fraction_leaf': 0.0})"
        ...
        >>> from rayml.pipelines.components.transformers.imputers.simple_imputer import SimpleImputer
        >>> assert generate_component_code(SimpleImputer()) == "from rayml.pipelines.components.transformers.imputers.simple_imputer import SimpleImputer\n\nsimpleImputer = SimpleImputer(**{'impute_strategy': 'most_frequent', 'fill_value': None})"
    """
