...
from rayml.automl.engine.cf_engine import CFClient, CFComputation, CFEngine
from rayml.automl.engine.engine_base import (
    JobLogger,
    evaluate_pipeline,
    train_pipeline,
)
from rayml.automl.engine.sequential_engine import SequentialEngine
from rayml.automl.utils import AutoMLConfig
from rayml.pipelines import BinaryClassificationPipeline
from rayml.pipelines.pipeline_base import PipelineBase
from rayml.tests.automl_tests.dask_test_utils import (
    DaskPipelineSlow,
...
def test_init(process_pool):
    with CFClient(process_pool) as client:
        ...
        with pytest.raises(
            TypeError,
            match="Expected rayml.automl.engine.cf_engine.CFClient, received",
        ):
...
def test_cfengine_convenience():
    ...
    with pytest.raises(
        TypeError, match="Expected rayml.automl.engine.cf_engine.CFClient, received"
    ):
