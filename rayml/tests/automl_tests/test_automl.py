...
from rayml.automl.automl_algorithm import IterativeAlgorithm
from rayml.automl.automl_search import build_engine_from_str
from rayml.automl.callbacks import (
    log_error_callback,
    raise_error_callback,
    silent_error_callback,
)
from rayml.automl.engine import CFEngine, DaskEngine, SequentialEngine
from rayml.automl.utils import (
...
from rayml.objectives.utils import (
...
from rayml.preprocessing import TimeSeriesSplit, TrainingValidationSplit
...
from rayml.tests.automl_tests.parallel_tests.test_automl_dask import (
    engine_strs,
)
from rayml.tests.conftest import CustomClassificationObjectiveRanges
from rayml.tuners import NoParamsException, RandomSearchTuner, SKOptTuner
...
@patch("rayml.objectives.AUC.score")
...
@patch("rayml.tuners.random_search_tuner.RandomSearchTuner.is_search_space_exhausted")
...
@patch("rayml.automl.automl_algorithm.DefaultAlgorithm.next_batch")
...
@patch("rayml.automl.automl_search.AutoMLSearch.search")
...
@patch("rayml.automl.engine.sequential_engine.SequentialComputation.get_result")
...
@patch("builtins.input")
@patch("rayml.automl.engine.sequential_engine.SequentialComputation.done")
...
@patch(
    "rayml.automl.engine.sequential_engine.SequentialComputation.done",
    side_effect=KeyboardInterruptOnKthPipeline(k=4, starting_index=2),
)
@patch("rayml.automl.engine.sequential_engine.SequentialComputation.cancel")
...
@patch("rayml.automl.automl_algorithm.IterativeAlgorithm.next_batch")
@patch("rayml.automl.AutoMLSearch.full_rankings", new_callable=PropertyMock)
@patch("rayml.automl.AutoMLSearch.rankings", new_callable=PropertyMock)
...
@patch("rayml.automl.engine.engine_base.split_data")
...
@patch("rayml.tuners.skopt_tuner.Optimizer.tell")
def test_percent_better_than_baseline_in_rankings(
    ...
    baseline_pipeline_class = {
        ProblemTypes.BINARY: "rayml.pipelines.BinaryClassificationPipeline",
        ProblemTypes.MULTICLASS: "rayml.pipelines.MulticlassClassificationPipeline",
        ProblemTypes.REGRESSION: "rayml.pipelines.RegressionPipeline",
        ProblemTypes.TIME_SERIES_REGRESSION: "rayml.pipelines.TimeSeriesRegressionPipeline",
    }[problem_type_value]
...
@patch("rayml.pipelines.BinaryClassificationPipeline.fit")
@patch("rayml.pipelines.MulticlassClassificationPipeline.fit")
@patch("rayml.pipelines.RegressionPipeline.fit")
@patch("rayml.pipelines.TimeSeriesRegressionPipeline.fit")
@patch("rayml.tuners.skopt_tuner.Optimizer.tell")
...
@patch("rayml.objectives.BinaryClassificationObjective.optimize_threshold")
...
@patch("rayml.automl.engine.sequential_engine.train_pipeline")
...
def test_automl_issues_beta_warning_for_time_series(problem_type, X_y_binary):
    ...
    with warnings.catch_warnings(record=True) as warn:
        ...
        assert len(warn) == 1
        message = "Time series support in rayml is still in beta, which means we are still actively building its core features"
...
@pytest.mark.parametrize("verbose", [True, False])
@patch(
    "rayml.objectives.binary_classification_objective.BinaryClassificationObjective.optimize_threshold",
    return_value=0.65,
)
...
@patch("rayml.objectives.standard_metrics.F1.optimize_threshold", return_value=0.42)
...
@patch("rayml.pipelines.utils._get_preprocessing_components")
@pytest.mark.parametrize("verbose", [True, False])
...
@patch(
    "rayml.pipelines.BinaryClassificationPipeline.score",
...
@patch("rayml.pipelines.components.RandomForestClassifier.predict")
@patch("rayml.pipelines.components.ElasticNetClassifier.predict_proba")
@patch("rayml.pipelines.components.ElasticNetClassifier.predict")
