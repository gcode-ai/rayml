...
from rayml.objectives import BinaryClassificationObjective
...
from rayml.pipelines.components.ensemble.stacked_ensemble_base import (
    _nonstackable_model_families,
)
from rayml.pipelines.components.utils import _all_estimators
        ...
        X_all = pd.DataFrame(
            {
                ...
                * 2,
                ...
                "text": [
                    ...
                    "rayml should handle string input",
                    "cats are gr8",
                    "hello world",
                    "rayml is gr8",
                ]
                ...
                * 2,
                "url": [
                    "https://rayml.alteryx.com/en/stable/",
                    ...
                    "https://www.rayml.alteryx.com/en/stable/demos/text_input.html",
                    "https://github.com/alteryx/rayml",
...
class _AutoMLTestEnv:
    ...
    def _pipeline_class(self):
        return {
            ...
            ProblemTypes.TIME_SERIES_MULTICLASS: "rayml.pipelines.TimeSeriesMulticlassClassificationPipeline",
            ProblemTypes.TIME_SERIES_BINARY: "rayml.pipelines.TimeSeriesBinaryClassificationPipeline",
        }[self.problem_type]
    ...
    def test_context(
        ...
        mock_get_names = patch(
            "rayml.pipelines.components.FeatureSelector.get_names", return_value=[]
        )
        ...
        if is_regression(self.problem_type):
            pipeline_to_mock = "rayml.pipelines.BinaryClassificationPipeline"
        ...
        mock_optimize = patch(
            "rayml.objectives.BinaryClassificationObjective.optimize_threshold",
            return_value=optimize_threshold_return_value,
        )

        mock_tell = patch("rayml.tuners.skopt_tuner.Optimizer.tell")
        ...
        mock_sleep = patch(
            "rayml.automl.AutoMLSearch._sleep_time", new_callable=sleep_time
        )
