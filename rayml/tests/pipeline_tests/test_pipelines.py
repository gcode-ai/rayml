...
from rayml.pipelines.component_graph import ComponentGraph
...
from rayml.pipelines.utils import _get_pipeline_base_class
from rayml.preprocessing.utils import is_classification
...
@patch("rayml.pipelines.RegressionPipeline.predict")
...
@patch("rayml.pipelines.ComponentGraph.fit")
@patch("rayml.pipelines.RegressionPipeline.predict")
...
@patch("rayml.pipelines.component_graph._schema_is_equal", return_value=True)
...
@patch("rayml.pipelines.MulticlassClassificationPipeline.fit")
@patch("rayml.pipelines.ComponentGraph.predict")
...
@patch("rayml.pipelines.MulticlassClassificationPipeline._encode_targets")
@patch("rayml.pipelines.MulticlassClassificationPipeline.fit")
@patch("rayml.pipelines.components.Estimator.predict")
@patch("rayml.pipelines.component_graph._schema_is_equal", return_value=True)
...
@patch("rayml.objectives.R2.score")
@patch("rayml.pipelines.RegressionPipeline.fit")
@patch("rayml.pipelines.RegressionPipeline.predict")
...
@patch("rayml.pipelines.BinaryClassificationPipeline._encode_targets")
@patch("rayml.objectives.F1.score")
@patch("rayml.pipelines.BinaryClassificationPipeline.fit")
@patch("rayml.pipelines.components.Estimator.predict")
@patch("rayml.pipelines.component_graph._schema_is_equal", return_value=True)
...
@patch("rayml.pipelines.MulticlassClassificationPipeline._encode_targets")
@patch("rayml.objectives.F1Micro.score")
@patch("rayml.pipelines.MulticlassClassificationPipeline.fit")
@patch("rayml.pipelines.components.Estimator.predict")
@patch("rayml.pipelines.component_graph._schema_is_equal", return_value=True)
...
@patch("rayml.pipelines.components.StandardScaler.transform")
...
def test_pipeline_not_fitted_error(
    ...
    if is_classification(problem_type):
        to_patch = "rayml.pipelines.ClassificationPipeline._predict"
        if problem_type == ProblemTypes.BINARY:
            to_patch = "rayml.pipelines.BinaryClassificationPipeline._predict"
...
@patch("rayml.pipelines.PipelineBase.fit")
@pytest.mark.parametrize(
    "problem_type",
    [ProblemTypes.BINARY, ProblemTypes.MULTICLASS, ProblemTypes.REGRESSION],
)
def test_nonlinear_pipeline_not_fitted_error(
    ...
    clf.fit(X, y)
    if problem_type in [ProblemTypes.BINARY, ProblemTypes.MULTICLASS]:
        with patch("rayml.pipelines.ClassificationPipeline.predict") as mock_predict:
            clf.predict(X)
            mock_predict.assert_called()
        with patch(
            "rayml.pipelines.ClassificationPipeline.predict_proba"
        ) as mock_predict_proba:
            clf.predict_proba(X)
            mock_predict_proba.assert_called()
    else:
        with patch("rayml.pipelines.RegressionPipeline.predict") as mock_predict:
            clf.predict(X)
            mock_predict.assert_called()
    clf.feature_importance
...
@patch("rayml.pipelines.components.LogisticRegressionClassifier.fit")
