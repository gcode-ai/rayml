...
from rayml.objectives import FraudCost, get_objective
...
@patch("rayml.pipelines.components.RandomForestRegressor.fit")
@patch("rayml.pipelines.components.RandomForestClassifier.fit")
@patch(
    "rayml.pipelines.TimeSeriesClassificationPipeline._encode_targets",
...
@patch("rayml.pipelines.components.RandomForestRegressor.predict")
...
@patch("rayml.pipelines.components.LogisticRegressionClassifier.predict")
@patch("rayml.pipelines.PipelineBase._score_all_objectives")
@patch("rayml.pipelines.TimeSeriesBinaryClassificationPipeline._score_all_objectives")
def test_ts_score(
    mock_binary_score,
    mock_score,
    mock_classifier_predict,
...
@patch("rayml.pipelines.LogisticRegressionClassifier.fit")
@patch("rayml.pipelines.LogisticRegressionClassifier.predict_proba")
@patch("rayml.pipelines.LogisticRegressionClassifier.predict")
@patch("rayml.pipelines.TimeSeriesClassificationPipeline._score_all_objectives")
...
@patch("rayml.objectives.BinaryClassificationObjective.decision_function")
@patch("rayml.pipelines.components.Estimator.predict_proba")
@patch(
    "rayml.pipelines.components.Estimator.predict",
...
def test_time_series_pipeline_not_fitted_error(
    ...
    if is_classification(problem_type):
        to_patch = "rayml.pipelines.TimeSeriesClassificationPipeline.predict_in_sample"
        if problem_type == ProblemTypes.TIME_SERIES_BINARY:
            to_patch = "rayml.pipelines.TimeSeriesBinaryClassificationPipeline.predict_in_sample"
...
@patch("rayml.objectives.FraudCost.decision_function")
...
@patch("rayml.pipelines.LogisticRegressionClassifier.fit")
@patch("rayml.pipelines.components.ElasticNetRegressor.fit")
