...
from rayml.__main__ import cli
from rayml.utils.cli_utils import (
    get_rayml_root,
...
def test_print_cli_info_cmd(caplog):
    ...
    assert result.exit_code == 0
    assert "rayml version:" in caplog.text
    assert "rayml installation directory:" in caplog.text
...
def test_print_info(caplog):
    print_info()
    out = caplog.text
    assert "rayml version:" in out
    assert "rayml installation directory:" in out
...
def test_get_rayml_root(current_dir):
    root = os.path.abspath(os.path.join(current_dir, "..", ".."))
    assert get_rayml_root() == root
