...
from rayml.exceptions import ValidationErrorCode
from rayml.model_understanding.graphs import visualize_decision_tree
from rayml.pipelines.components import ComponentBase
from rayml.utils.gen_utils import (
    SEED_BOUNDS,
...
def test_import_or_raise_errors(dummy_importlib):
    ...
    with pytest.raises(ImportError, match="Missing optional dependency '_rayml'"):
        import_or_raise("_rayml")
    with pytest.raises(
        ImportError,
        match="Missing optional dependency '_rayml'. Please use pip to install _rayml. Additional error message",
    ):
        import_or_raise("_rayml", "Additional error message")
    with pytest.raises(
        Exception,
...
def test_import_or_warn_errors(dummy_importlib):
    ...
    with pytest.warns(UserWarning, match="Missing optional dependency '_rayml'"):
        import_or_raise("_rayml", warning=True)
    with pytest.warns(
        UserWarning,
        match="Missing optional dependency '_rayml'. Please use pip to install _rayml. Additional error message",
    ):
        import_or_raise("_rayml", "Additional error message", warning=True)
    with pytest.warns(
        UserWarning,
        match="An exception occurred while trying to import `attr_error_lib`: Mock Exception executed!",
    ):
...
@patch("rayml.utils.gen_utils.import_or_raise")
